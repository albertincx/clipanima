...
A feature-rich drawing canvas application built with React and TypeScript that supports animation creation with multiple frames and GIF export functionality.
...
- Lazy loading for ffmpeg.wasm module (loads only on first export)

### GIF Export Process
1. All frames are converted to base64 images
2. ffmpeg.wasm processes the images into an animated GIF
