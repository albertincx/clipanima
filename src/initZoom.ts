// @ts-nocheck
// Source - https://stackoverflow.com/a/45122001
...
const initZoom = (canvasId = 'canvas') => {
    ...
    (window as any).drawingCanvas = drawing;
    (window as any).getDrawingCanvasData = () => drawing.toDataURL();
    (window as any).setDrawingCanvasData = (dataUrl: string) => {
        const img = new Image();
        img.onload = () => {
    ...
    function drawOnDrawing() {  // draw all points on drawingPoint array
        ...
        dCtx.lineWidth = size; // Set line width for strokes
